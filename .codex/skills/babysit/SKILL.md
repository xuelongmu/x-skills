...
6. Run the shared watcher from the sibling `land` skill:
   ...
   Never set `LAND_WATCH_DISABLE_AUTO_MERGE` here: that variable makes the shared watcher cancel an auto-merge request, which only `land` may do. Babysitting reports an armed auto-merge and leaves it in place.
...
- `5`: PR is behind its base, has merge conflicts, has a dirty merge state, or was retargeted
