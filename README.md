...
- The shared Codex PR watcher polls GitHub every 30 seconds by default; set `LAND_WATCH_POLL_SECONDS` from 30 to 300 seconds when a repository needs a lower API request rate. Both hosts require synchronous final CI, PR-head, and merge-state refreshes after the 10-minute feedback grace window.
