...
- The shared Codex PR watcher polls GitHub every 30 seconds by default; set `LAND_WATCH_POLL_SECONDS` from 10 to 300 seconds when a repository needs a different API request rate. The upper bound preserves repeated CI checks during the 10-minute feedback grace window.
