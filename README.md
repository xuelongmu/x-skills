...
- The shared Codex PR watcher polls GitHub every 30 seconds by default; set `LAND_WATCH_POLL_SECONDS` from 30 to 300 seconds when a repository needs a lower API request rate. Both hosts require final feedback and PR snapshots to converge unchanged, with CI revalidated between them, after the 10-minute feedback grace window.
